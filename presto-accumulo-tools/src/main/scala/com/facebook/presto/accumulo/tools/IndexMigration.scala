...
class IndexMigration extends Task with Serializable {
  ...
  private val INSTANCE_OPT: Char = 'i'
  private val ZOOKEEPERS_OPT: Char = 'z'
  private val USER_OPT: Char = 'u'
  private val PASSWORD_OPT: Char = 'p'
  ...
  def getOptions: Options = {
    ...
    opts.addOption(OptionBuilder.create(INSTANCE_OPT))

    OptionBuilder.withLongOpt("zookeepers")
    OptionBuilder.withDescription("Accumulo ZooKeeper connect string")
    OptionBuilder.isRequired
    OptionBuilder.hasArg
    opts.addOption(OptionBuilder.create(ZOOKEEPERS_OPT))

    OptionBuilder.withLongOpt("user")
    OptionBuilder.withDescription("Accumulo username")
    OptionBuilder.isRequired
    OptionBuilder.hasArg
    opts.addOption(OptionBuilder.create(USER_OPT))

    OptionBuilder.withLongOpt("password")
    OptionBuilder.withDescription("Accumulo password")
    OptionBuilder.isRequired
    OptionBuilder.hasArg
    opts.addOption(OptionBuilder.create(PASSWORD_OPT))
  ...
  override def run(conf: AccumuloConfig, cmd: CommandLine): Int = {
    val instance = cmd.getOptionValue(INSTANCE_OPT)
    val zooKeepers = cmd.getOptionValue(ZOOKEEPERS_OPT)
    val username = cmd.getOptionValue(USER_OPT)
    val password = cmd.getOptionValue(PASSWORD_OPT)
    ...
    ConfiguratorBase.setConnectorInfo(classOf[AccumuloInputFormat], conf, username, new PasswordToken(password))
    InputConfigurator.setScanAuthorizations(classOf[AccumuloInputFormat], conf, auths)
    InputConfigurator.setInputTableName(classOf[AccumuloInputFormat], conf, srcTableName)

    val rdd = spark.sparkContext.newAPIHadoopRDD(conf, classOf[AccumuloInputFormat], classOf[Key], classOf[Value])

    // Group entries by key and then write each partition to Accumulo via the PrestoBatchWriter
    rdd.groupBy(entry => entry._1.getRow)
      .repartition(numPartitions)
      .foreachPartition(partition => writePartition(instance, zooKeepers, username, password, destTableName, auths, partition))
